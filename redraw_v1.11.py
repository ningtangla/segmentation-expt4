...
def mouse_select(event, x, y, flag, param):
    global CLICK_NUMBER, CLICK_POINTS, CONFIRM_POINTS, COLORFILL_NUMBER, CLICK_COLOR
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if CLICK_NUMBER == 1:
            ...
            for z in range(len(CONFIRM_POINTS)):
                cv2.circle(param[0], (int(CONFIRM_POINTS[z][0]), int(CONFIRM_POINTS[z][1])), CIRCLE_SIZE, (230, 80, 25), -1)
            ...
            if CLICK_NUMBER == 2:
                ...
                cv2.imshow('image', param[0])
                cv2.imwrite(data_path+str(IMAGE_ORDER[param[3] - 1])+'/'+str(CUT_NUMBER - param[4] + 1)+'.png', param[0])
                selected_shape = select_cutshape(param[2], CLICK_POINTS)
                points_reallocation(cut_points = CLICK_POINTS, vertexes = param[2], selected_shape = selected_shape)
                CLICK_POINTS = []
                CONFIRM_POINTS = []
        ...
        if CLICK_NUMBER == 2:
            for j in range(len(COLOR_ALL)):
                if (x > 0.9*image_width) and (x < 1.0*image_width) and (y > (0.5+j)*0.1*image_height) and (y < (1.5+j)*0.1*image_height):
                    ...
                    CLICK_NUMBER = CLICK_NUMBER + 1
                    cv2.rectangle(param[0], (int(0.9*image_width), int((0.5+j)*0.1*image_height)), (int(1.0*image_width), int((1.5+j)*0.1*image_height)), (40, 40, 40), -1)
                    break
            cv2.imshow('image', param[0]) 
        
        if CLICK_NUMBER == 3:
            for j in range(len(param[2])):
                min_x = min(param[2][j][0][0], param[2][j][2][0])
                max_x = max(param[2][j][0][0], param[2][j][2][0])
                min_y = min(param[2][j][0][1], param[2][j][2][1])
                max_y = max(param[2][j][0][1], param[2][j][2][1])
                if (x > min_x) and (x < max_x) and (y > min_y) and (y < max_y):
                    cv2.rectangle(param[0], (int(min_x), int(min_y)), (int(max_x), int(max_y)), CLICK_COLOR[COLORFILL_NUMBER], -1)
                    COLORFILL_NUMBER = COLORFILL_NUMBER + 1
                    press_image = cv2.imread(image_path+'press1.png')
                    param[0][68:100, 428:600] = press_image
                    break
            cv2.imshow('image', param[0])
            cv2.imwrite(colordata_path+str(IMAGE_ORDER[param[3] - 1])+'/'+str(COLORFILL_NUMBER)+'.png', param[0])
            
                              
def parse(vertexes, image, image_number, parse_order):
    ...
    cv2.setMouseCallback('image', mouse_select, [image, possible_points, vertexes, image_number, parse_order])
    cv2.waitKey()
...
def color_fill(event, x, y, flag, param):
    global CLICK_COLOR, CLICK_NUMBER, COLORFILL_NUMBER
    if event == cv2.EVENT_LBUTTONDOWN:
        if CLICK_NUMBER == 0:
            for j in range(len(COLOR_ALL)):
                if (x > 0.9*image_width) and (x < 1.0*image_width) and (y > (0.5+j)*0.1*image_height) and (y < (1.5+j)*0.1*image_height):
        ...
        if CLICK_NUMBER == 1:
            for j in range(len(param[1])):
                ...
                if (x > min_x) and (x < max_x) and (y > min_y) and (y < max_y):
                    ...
                    break
            cv2.imshow('image', param[0])
            cv2.imwrite(colordata_path+str(IMAGE_ORDER[param[2] - 1])+'/'+str(COLORFILL_NUMBER)+'.png', param[0])                 
...
def cut(vertexes, cut_order, image, image_number):
    global COLORFILL_NUMBER, CLICK_NUMBER
    
    for i in range(CUT_NUMBER + 1 - cut_order):
        points = np.array(vertexes[i], np.int32)
        cv2.polylines(image, [points], True, (155,255,155), 3)
        cv2.imshow('image', image)         
    
    image = parse(vertexes = vertexes, image = image, image_number = image_number, parse_order = cut_order)[1]
...
#        image[:] = 40
        ...
        while COLORFILL_NUMBER < CUT_NUMBER + 1:
            cv2.setMouseCallback('image', color_fill, [image, vertexes, image_number])
            cv2.waitKey()
            image[68:100, 428:600] = 40
            cv2.imshow('image', image)
            CLICK_NUMBER = 0
